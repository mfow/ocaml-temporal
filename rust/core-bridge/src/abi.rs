...
impl Runtime {
    ...
    /// workflow service. The request is decoded before any connection lookup;
    /// the acknowledgement is strictly encoded before crossing the ABI.
...
/// Request immediate termination of one exact workflow run.
///
/// The successful value is a strict `{"acknowledged":true}` document. The
/// request is bounded to the synchronous owner-Domain call and never retains
/// an OCaml pointer after return.
///
/// # Safety
///
/// `runtime` must be a live, exclusively owned runtime handle. The input span
...
/// initialized-result contract.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ocaml_temporal_core_v1_client_terminate_workflow_json(
    runtime: *mut Runtime,
    input: *const u8,
    input_len: usize,
    output: *mut Result,
