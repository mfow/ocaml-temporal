...
- **Verified live** means a test used the real Temporal Server and PostgreSQL
  ...
  retry, and one typed non-retryable workflow failure. The live retry
  assertion passed in CI run
  [`29187733405`](https://github.com/mfow/ocaml-temporal/actions/runs/29187733405)
  for commit `b895d3c`. Exact-run cancellation is implemented and focused
  tested; the current acceptance change adds its live exact-run scenario and
  requires its own CI evidence.
...
| Public client start, exact-run wait, and cancellation | A separate OCaml driver starts six top-level workflows through `Temporal.Client`, retains all returned handles, acknowledges cancellation for the long-running handle, and waits for each exact workflow/run pair. | **Pending this change's CI run.** The historical run [`29187733405`](https://github.com/mfow/ocaml-temporal/actions/runs/29187733405) verifies the four earlier successful terminal results; this change adds the fifth typed-failure assertion and exact-run cancellation. Continued-as-new remains untested live. |
...
| Native client start, exact-run wait, and cancellation | The private [client protocol](client-protocol.md), Rust client bridge tests, supervisor operations, request correlation, bounded waits, positive cancellation acknowledgement, terminal outcome mapping, and typed public handles are implemented. Cancellation uses the exact retained run identity and a bounded native RPC. | **Pending this change's CI run.** The real client starts six top-level workflows; four return exact payloads, one returns a typed non-retryable workflow failure, and `two-binary-long-running-cancellation` returns typed `Cancelled` after the driver cancels its exact handle. Continued-as-new remains untested live. |
| Native workflow poll and completion | The workflow protocol, [native execution translation](native-execution-translation.md), private worker registry, readiness waits, command validation, activation ordering, timers, cancellation, eviction, and completion retry are implemented and focused-tested. | **Pending this change's CI run.** The live worker runs registered OCaml workflows through successful completion, turns the long-running timer cancellation into a terminal cancel command, and shuts down through the public API after the driver finishes. Eviction and recovery remain focused-test evidence. |
...
| Native readiness and lifecycle ownership | Rust readiness lanes, bounded waits that release the OCaml runtime lock, one owner-Domain supervisor, C/Rust response ownership, cleanup, and ABI checks are documented in [Core bridge](core-bridge.md) and covered by bridge, Rust, and supervisor tests. | **Pending this change's CI run.** The live worker loop runs under the initial task load, cancellation is observed while work is outstanding, and the worker shutdown marker is checked after its signal watcher and public shutdown operation complete. |
