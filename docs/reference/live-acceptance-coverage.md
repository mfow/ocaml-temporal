...
That run provides evidence for those concrete happy paths only. The current
acceptance scenario additionally exercises exact-run cancellation, but its
real-server evidence is established by the CI run for this change rather than
by the historical run linked above. Retry timeouts, activity-level
...
workflow, sends `Temporal.Client.cancel` using its returned exact handle, then
waits on that same handle and checks the `Cancelled` category, retryability,
...
| A workflow starts, runs, and returns a terminal result | Synthetic activation and native adapter tests cover command construction and terminal handling. | **Pending this change's CI run.** The driver retains exact workflow/run handles and starts six top-level workflows before the first wait. Four return exact payloads, `smoke.non_retryable_failure` returns a typed non-retryable workflow failure, and `smoke.long_running_cancellation` is cancelled through its exact handle. | Timed-out, terminated, and continued-as-new outcomes remain untested live. |
...
| Multiple operations scheduled before awaiting | **Verified (synthetic only).** Scheduler and activation tests cover completion ordering, first-error behavior, and cancellation semantics. | **Pending this change's CI run.** `smoke.fan_out` creates two mock-activity futures before its first wait, while the driver starts all six executions and acknowledges cancellation before its first terminal wait. | `race` and explicit server-history ordering assertions are **Planned — later expansion**. |
| Typed workflow failures and non-success terminal outcomes | **Verified (synthetic and one live workflow-failure path).** Error, protocol, client, worker, and runtime tests check typed rejection and terminal state handling without exceptions for expected failures. | **Pending this change's CI run.** `smoke.non_retryable_failure` returns a deterministic `Temporal.Error.t` with category `Workflow`, `non_retryable=true`, and the stable message prefix `intentional terminal workflow failure`; the driver also requires the exact cancellation metadata for `smoke.long_running_cancellation`. | Timeout, termination, continued-as-new, and child-failure outcomes remain **Planned — later expansion**. |
