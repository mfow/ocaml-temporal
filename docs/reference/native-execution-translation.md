...
| `Start_child_workflow` | `Start_child_workflow` | The sequence, child workflow ID and type, one copied input payload, and explicit child-cancellation policy are retained. Namespace, task queue, workflow timeouts, close/reuse policies, retry, cron, headers, memo, search attributes, versioning, and priority are not represented by the current OCaml command: Rust sends their Core defaults, and its reverse conversion rejects a non-default Core value instead of silently discarding it. |
