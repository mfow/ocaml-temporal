...
workflows. The current driver starts ten workflows before it waits for any
result: `smoke.fan_out`, `smoke.timer_then_activity`,
`smoke.continue_as_new`, `smoke.activity_retry`, `smoke.activity_heartbeat_retry`,
...
head `47c9a93`; that PR was squash-merged as `f877fbf`. It live-verified the
first nine scenarios; the new continue-as-new successor-follow scenario is
...
the current ten-scenario fixture; the continuation scenario is not live
evidence until its complete gate passes.
...
is the authoritative live evidence for the original nine scenarios listed
above. The continue-as-new scenario becomes authoritative only after its own
complete smoke run passes. The target cleans the Compose project and
...
retry-timeout, restart, replay, or recovery scenarios.
