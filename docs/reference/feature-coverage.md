...
| Signals, queries, updates, validators, conditions, and handler policies | **Experimental typed definitions and deterministic local dispatch are implemented.** `Temporal.Signal`, `Temporal.Query`, and `Temporal.Update` pair names with codecs and typed handlers; `Temporal.Interaction` tests duplicate registration, codec boundaries, validator-before-handler ordering, and exception containment. Native Temporal interaction activation/completion, conditions, and handler policies are not implemented yet. | [Interactive workflow reference](interactive-workflows.md), [Roadmap Phase 5](../implementation-roadmap.md#delivery-order) |
