...
- **Verified live** means a test used the real Temporal Server and PostgreSQL
  Compose stack. At this baseline, the complete [PR #210 CI run](https://github.com/mfow/ocaml-temporal/actions/runs/29221151859)
  verifies the two-binary path for nine scenarios: four exact successes,
  ordinary activity retry, heartbeat-detail retry, parent/child success,
  propagated child failure, child cancellation, typed non-retryable workflow
  failure, and exact-run cancellation. The PR run used head `47c9a93` and was
  squash-merged as `f877fbf`.
...
| Public client start, exact-run wait, and cancellation | A separate OCaml driver starts nine top-level workflows through `Temporal.Client`, retains all returned handles, waits for a marker activity before acknowledging cancellation for the long-running handle, and waits for each exact workflow/run pair. | **Verified live** in the [PR #210 run](https://github.com/mfow/ocaml-temporal/actions/runs/29221151859). Continued-as-new remains untested live. |
| Public worker workflow and activity dispatch | A separate OCaml worker registers the workflows and activities, including ordinary retry, context-aware heartbeat retry, child-failure/cancellation, typed-failure, and cancellation-marker definitions, through real Core polling/completion. | **Verified live** in the [PR #210 run](https://github.com/mfow/ocaml-temporal/actions/runs/29221151859). Activity timeout and asynchronous completion remain deferred. |
...
| Temporal Core timeout failure information | `TimeoutFailureInfo` is mapped to a closed semantic timeout record with the exact timeout policy and ordered last-heartbeat payloads. Rust and OCaml validate payload limits and reject unknown timeout enum values; the JSON shape is in [`temporal-failure.schema.json`](../schemas/bridge/temporal-failure.schema.json). Focused Rust and OCaml round-trip tests cover populated and absent heartbeat details. | The mapping now accepts timeout failures that previously stopped the bridge. No new live smoke success is claimed here; timeout-triggered activity retry remains separately tracked by the live acceptance matrix. |
| Private replay worker and history feeder | **Unit-tested native plumbing.** `rust/core-bridge/src/replay_bridge.rs` validates the closed replay-history JSON document, canonical base64 protobuf, and Core history invariants, then feeds histories through a one-slot workflow-only `HistoryFeeder`; focused tests live in [`rust/core-bridge/tests/support/replay_bridge.rs`](../../rust/core-bridge/tests/support/replay_bridge.rs). | No live evidence. The public C/OCaml replay operation, worker replacement, replay marker, and fresh-volume Compose acceptance scenario remain planned. |
...
| Native readiness and lifecycle ownership | Rust readiness lanes, bounded waits that release the OCaml runtime lock, one owner-Domain supervisor, C/Rust response ownership, cleanup, and ABI checks are documented in [Core bridge](core-bridge.md) and covered by bridge, Rust, and supervisor tests. | The [PR #210 run](https://github.com/mfow/ocaml-temporal/actions/runs/29221151859) checks the nine-run path, marker-guarded cancellation while work is outstanding, and both client and worker shutdown markers. |
