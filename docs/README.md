...
| Public native worker | Focused adapter, supervisor, Rust bridge, lifecycle tests, and a real two-binary Compose path covering four successes plus one typed workflow failure | Cancellation, child failure/cancellation, replay, and recovery scenarios remain untested live |
| Public native client | Typed start/wait protocol plus five real two-process Compose terminal assertions | Cancellation, continued-as-new, and other client commands remain untested live |
