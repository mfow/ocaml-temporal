...
| Exact-run output-only client query while a workflow is suspended | **Verified (synthetic only).** Public query definitions, client request/response conversion, query-only activation handling, and read-only/non-suspending capability checks have focused OCaml and Rust coverage. The two-binary source contract also requires the client to query the signal-condition execution and the worker to register its handler. | **Pending the first successful live run.** The ordinary two-binary controller now waits until `smoke.signal_condition` has parked on its deterministic condition, queries that exact run for `SMOKE:QUERY:PENDING`, then sends its typed signal and checks the ordinary terminal result. The ordering prevents a completed workflow or final signal state from standing in for a live query activation. | This covers only the output-only API. Typed query inputs, query rejection/error paths, query behavior across replay or cache eviction, and query deadlines remain separately unverified. |
