...
- `Temporal.Worker` registers workflows, activities, and the signal, query, and
  ...
  supervisor graph, runs the poll loops, and performs idempotent shutdown. After
  a successful shutdown, calling `Worker.run` again on the same instance returns
  a typed `bridge` error; create a new worker when a new polling lifecycle is
  needed. The interaction handler registration modes and their current native
  limitations are described in the [interactive-workflow reference](interactive-workflows.md).
