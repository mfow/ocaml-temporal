...
(** Exact workflow/run pair and operator reason for immediate termination. *)
...
(** Converts a public termination request to the namespace-bound protocol
    value. Termination has no request ID: the exact workflow/run identity is
    the idempotency key for this destructive control operation. *)
let native_terminate_request client (request : terminate_request) :
    Client_protocol.terminate_request =
  {
    execution =
      {
        namespace = client.namespace;
...
(** Requests immediate termination through the serialized supervisor. The
    acknowledgement confirms RPC acceptance; [client_wait] observes the
    resulting terminal state. *)
let native_client_terminate (client : native_client) (request : terminate_request) :
    (unit, Error.t) result =
  if Atomic.get client.closed then Error (bridge_error "client is shut down")
  else
    match
      Native.perform client.supervisor
        (Native.Client_terminate_workflow
           (native_terminate_request client request))
    with
    | Error error -> Error (native_supervisor_error error)
    | Ok (Error error) -> Error (native_client_error error)
    | Ok (Ok ()) -> Ok ()
...
(** Marks one exact mock execution terminated. Repeated requests are
    idempotent, while completed or cancelled history remains immutable. *)
...
(** Requests termination on the selected private transport. *)
