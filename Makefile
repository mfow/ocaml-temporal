...
# driver's ten top-level executions and the deterministic child created by its
# parent workflow. The heartbeat-retry and timeout-retry IDs are also listed so
# activity heartbeat/detail or timeout failure is visible in best-effort
# diagnostics. The cancellation ID is intentionally listed explicitly so a
...
temporal-inspect-smoke:
	@for workflow_id in two-binary-fan-out two-binary-timer-then-activity two-binary-activity-retry two-binary-activity-heartbeat-retry two-binary-activity-timeout-retry two-binary-parent-awaits-child two-binary-parent-awaits-failed-child two-binary-parent-cancels-child two-binary-non-retryable-failure two-binary-long-running-cancellation; do \
