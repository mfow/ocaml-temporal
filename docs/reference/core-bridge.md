...
There is no `follow_runs` escape hatch in the document: every call uses a
close-event history long poll for that exact run, but the native operation is
bounded to 100 ms. If no close event arrives during that interval, the ABI
...
from accidentally observing a different execution identity.
