...
| Public client start, exact-run wait, and cancellation | A separate OCaml driver starts eleven top-level workflows before its first wait through `Temporal.Client`, retains all returned handles, then starts the timeout-retry workflow after the heartbeat result. It waits for every exact workflow/run pair and waits for the marker activity before acknowledging cancellation for the long-running handle. | **Verified live locally** by `OCAML_VERSION=5.5 DUNE_JOBS=1 make test-temporal-integration`, including the continue-as-new successor and exact-run cancellation. CI evidence for this newer twelve-result path is still pending. |
...
| Native client start, exact-run wait, and cancellation | The private [client protocol](client-protocol.md), Rust client bridge tests, supervisor operations, request correlation, bounded waits, positive cancellation acknowledgement, terminal outcome mapping, and typed public handles are implemented. Cancellation uses the exact retained run identity and a bounded native RPC. | **Verified live locally** for start, exact-run wait, marker-guarded cancellation, and continue-as-new successor following. CI evidence for the newer twelve-result path remains pending. |
