...
| Workflow patch marker decisions | **Verified (synthetic only).** Runtime tests cover a new execution, replay with and without `NotifyHasPatch`, repeated calls, run isolation, and patch-ID copying. Shared fixtures plus Rust tests cover strict JSON and pinned Core round trips. | **Not yet live-verified.** | Add two real histories: an older history created before the marker and a newer history containing it, then replay both through a replacement worker and assert the old/new branches respectively. Patch deprecation/removal remains a later scenario. |
