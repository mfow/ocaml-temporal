...
| `temporal-integration` | `make test-temporal-integration`, `make test-temporal-worker-restart`, `make test-temporal-worker-crash-recovery`, `make test-temporal-worker-cache-eviction`, `make test-temporal-workflow-patching`, and `make test-temporal-parent-child-restart` | Run the corresponding target when Docker and network access are available | Six sequential live controllers reuse one job's checkout/build cache while each owns a fresh Temporal/PostgreSQL lifecycle. The 45-minute CI ceiling includes the bilateral parent/child recovery gate; contract-only results are not live evidence. |
