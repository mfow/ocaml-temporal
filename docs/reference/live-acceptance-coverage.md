...
The six-run driver and marker-guarded cancellation are implemented and covered
by local acceptance checks, but they are not live-verified yet: the PR #85
[Actions run](https://github.com/mfow/ocaml-temporal/actions/runs/29193818312)
was cancelled before producing a green result. The driver waits for a
...
| Exact-run client cancellation and graceful shutdown with outstanding work | **Verified (synthetic only).** The public mock client, supervisor, OCaml bridge protocol, and Rust protocol tests validate exact run identity, bounded request handling, positive acknowledgement, stable request IDs, and typed `Cancelled` observation. The six-run driver also has local marker and result assertions. | **Implemented, but not live-verified.** The driver cancels `two-binary-long-running-cancellation`, waits for its exact run, requires category `Cancelled`, `non_retryable=false`, and the stable cancellation message, then checks the driver's client-shutdown marker and the worker's graceful-shutdown marker. The PR #85 [Actions run](https://github.com/mfow/ocaml-temporal/actions/runs/29193818312) was cancelled, so no green CI run currently proves this path against a live server. | Cancellation of child/activity work, restart, replay, and cache eviction remain **Planned — later expansion**. |
