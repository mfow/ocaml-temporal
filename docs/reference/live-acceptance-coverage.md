...
| Workflow patch marker decisions | **Verified (synthetic only).** Runtime tests cover a new execution, replay with and without `NotifyHasPatch`, repeated calls, run isolation, and patch-ID copying. Shared fixtures plus Rust tests cover strict JSON and pinned Core round trips. | **Implemented — live verification pending.** `make test-temporal-workflow-patching` first runs a Docker-free fixture contract, then is configured to create a marker-free legacy history and a marker-bearing new history, replace the worker for each, observe replay diagnostics, and require the respective old/new branch result. No successful real-server invocation is recorded. | Patch deprecation/removal, deployment/build-ID versioning, and broader historical-compatibility coverage remain later scenarios. |
...
is the current complete CI evidence for the preceding seventeen-result baseline.
