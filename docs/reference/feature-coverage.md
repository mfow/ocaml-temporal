...
- **Verified live** means a test used the real Temporal Server and PostgreSQL
  ...
  for commit `b895d3c`. Exact-run cancellation is implemented and
  focused-tested but remains outside this live gate.
...
| Public client start and exact-run wait | A separate OCaml driver starts five top-level workflows through `Temporal.Client`, retains all returned handles, and waits for each exact workflow/run pair. | CI run [`29187733405`](https://github.com/mfow/ocaml-temporal/actions/runs/29187733405) verifies the four earlier successful terminal results; the full acceptance command for this change also passed locally with the fifth typed-failure assertion. The change's CI run is pending. Client cancellation and continued-as-new remain untested live. |
...
| Native client start, exact-run wait, and cancellation | The private [client protocol](client-protocol.md), Rust client bridge tests, supervisor operations, request correlation, bounded waits, positive cancellation acknowledgement, terminal outcome mapping, and typed public handles are implemented. Cancellation uses the exact retained run identity and a bounded native RPC. | The real client starts five top-level workflows; four return exact payloads and the fifth returns a typed non-retryable workflow failure. CI run [`29187733405`](https://github.com/mfow/ocaml-temporal/actions/runs/29187733405) remains evidence for the four earlier successes, and the full acceptance command for this change passed locally with the fifth assertion. The change's CI run is pending. Client cancellation and continued-as-new remain untested live. |
...
| --- | --- | --- |
| Live cancellation, child failure/cancellation, restart, replay, and cache-eviction scenarios | **Deferred as acceptance scenarios.** The two-binary gate now proves one typed non-retryable workflow failure and one activity that fails once before a server-managed retry; broader failure and recovery cases remain planned. | [Live acceptance coverage](live-acceptance-coverage.md) |
