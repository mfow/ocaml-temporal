...
- [x] **Step 2: Verify the tests fail for missing functions**
...
- [x] **Step 4: Verify future tests pass**
...
- [x] **Step 2: Verify the tests fail for missing `start_sleep`**

  Run: `opam exec -- dune runtest --root . test/runtime`

  Expected: compilation fails because `Temporal.Workflow.start_sleep` is absent.

- [x] **Step 3: Implement `start_sleep` and compose `sleep`**
...
- [x] **Step 4: Verify timer tests pass**
...
- [x] **Step 1: Write failing child-workflow tests**
...
- [x] **Step 2: Verify the tests fail for missing child operations**
...
- [x] **Step 3: Implement the private child command path**
...
- [x] **Step 4: Implement the public child module**
...
- [x] **Step 5: Verify child tests pass**
...
- [x] **Step 1: Write a failing helper-composition test**
...
- [x] **Step 4: Run all applicable local gates**
  ...
  Result: every available local check passed. The host did not have the pinned
  `cargo-deny`, `cargo-machete`, or `typos` executables, so their configured
  GitHub Actions jobs remain the scanner gate for this branch.
