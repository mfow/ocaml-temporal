...
module Local = struct
  ...
  type 'a t = {
    key : 'a Temporal_runtime.Workflow_context_store.local;
  }
  ...
  let create () =
    { key = Temporal_runtime.Workflow_context_store.create_local () }
  ...
  let get local =
    ...
    | Some context ->
        Ok (Temporal_runtime.Workflow_context_store.get_local context local.key)
  ...
  let set local value =
    ...
    | Some context ->
        Temporal_runtime.Workflow_context_store.set_local context local.key value;
        Ok ()
end
