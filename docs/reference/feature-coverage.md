...
- **Verified live** means a test used the real Temporal Server and PostgreSQL
  ...
  activity fan-out, a timer-then-activity workflow, and one parent awaiting a
  timer-owning child workflow. The activity-retry scenario is implemented but
  remains pending live CI verification for this revision.
...
| Public client start and exact-run wait | A separate OCaml driver starts the previously verified three top-level workflows plus the newly implemented retry workflow through `Temporal.Client`, retains all returned handles, and waits for each exact workflow/run pair. | The retry workflow remains pending live CI verification; non-success terminal outcomes and continued-as-new are not covered live. |
| Public worker workflow and activity dispatch | A separate OCaml worker registers the previously verified workflows and activities plus the newly implemented retry workflow/activity through real Core polling/completion. | The retry path remains pending live CI verification; activity timeout, heartbeat, asynchronous completion, non-retryable failure, and child failure/cancellation are not covered live. |
...
| Activity retry delivery | `smoke.activity_retry` schedules `smoke.retry_once` with an explicit two-attempt policy. The worker intentionally fails the first attempt and the driver requires the exact `SMOKE:ATTEMPT:2` result from the second task. | **Implemented; pending live CI verification.** Once green, this will prove one short success-after-retry path only; it will not prove timeout-triggered retries, non-retryable error-type matching, long backoff, or retry across worker restart. |
...
| Native client start and exact-run wait | The private [client protocol](client-protocol.md), Rust client bridge tests, supervisor operations, request correlation, bounded waits, terminal outcome mapping, and typed public handles are implemented. | The real client starts the previously verified three top-level workflows and the newly implemented retry workflow; the fourth workflow's live result remains pending this revision's CI. Non-success outcomes and continued-as-new remain untested live. |
...
| Native remote-activity poll and completion | The [activity protocol](activity-protocol.md), private activity adapter, copied opaque-token lease, cancellation completion, strict validation, and retryable drain are implemented and focused-tested. | The live worker completes the mock activities used by the fan-out and timer scenarios. The retry activity is implemented but its one-failure/second-attempt result remains pending this revision's live CI; heartbeats, asynchronous completion, timeout, and non-retryable failure remain deferred or synthetic-only. |
