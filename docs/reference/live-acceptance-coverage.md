...
normalized history, and volume cleanup are now live evidence. The separate
sticky-cache controller and Docker-free contract are implemented, but its
first successful live Actions run is still pending; crash recovery remains a
separate scenario.
...
| Worker restart, replay, sticky-cache eviction, and continued execution | **Verified (live restart/replay path); implemented (cache eviction contract).** Runtime tests cover replay-stable commands and cache eviction. `make test-temporal-worker-cache-eviction-contract` validates a dedicated one-entry-cache worker, separate OCaml client, exact `CacheFull` acknowledgement record, and normalized-history fixtures without Docker. | **Verified live** in the [PR #253 run](https://github.com/mfow/ocaml-temporal/actions/runs/29286560471) for restart/replay. `make test-temporal-worker-cache-eviction-live` is wired into CI but has no successful run recorded yet. | The cache controller must record its first successful real `RemoveFromCache(CacheFull)` run; crash recovery remains separate. |
...
make test-temporal-worker-cache-eviction # contract plus one-entry-cache CacheFull replay
```
...
cache-eviction, or recovery scenarios. `make test-temporal-worker-cache-eviction`
uses the same real server topology but a separate one-entry worker and is
currently implementation/contract evidence only. The [PR #253 run](https://github.com/mfow/ocaml-temporal/actions/runs/29286560471)
is the current complete CI evidence for the baseline and restart/replay paths.
