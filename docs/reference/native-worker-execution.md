...
6. `Native_execution` converts the command batch to a checked semantic
   completion. Activity and child-workflow commands remain explicit
   `unsupported` errors until the protocol has all of their Core fields; no
   task queue, timeout, ID, or cancellation default is invented.
...
runtime (for example, an activity command with missing Core fields) receives a
typed `Fail_workflow` completion. `poll` returns `Ok (Rejected ...)` only after
that completion has been accepted, and marks `lease_retired = true`. If the
native completion operation fails, `poll` returns an error and leaves the run
entry in place so the caller can retry or shut down without a false claim of
retirement.
...
- explicit rejection and lease retirement for unsupported activity commands;
