...
# driver's nine top-level executions and the deterministic child created by its
...
temporal-inspect-smoke:
	@for workflow_id in two-binary-fan-out two-binary-timer-then-activity two-binary-activity-retry two-binary-activity-heartbeat-retry two-binary-parent-awaits-child two-binary-parent-awaits-failed-child two-binary-parent-cancels-child two-binary-non-retryable-failure two-binary-long-running-cancellation; do \
