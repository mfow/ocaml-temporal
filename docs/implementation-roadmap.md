...
| 6 | Continue-as-new, patches, side effects, external workflow operations, memo, search attributes, priority, and fairness | Recorded histories replay and advanced command integration tests pass | In progress: continue-as-new is implemented and live-verified. The first non-deprecated `Temporal.Workflow.patched` primitive, bilateral JSON validation, per-execution replay decisions, and Core conversion are focused-tested; live old/new-history replay, patch deprecation/removal, side effects, external operations, memo/search attributes, priority, fairness, and broader CI evidence remain pending |
