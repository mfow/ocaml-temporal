...
| --- | --- | --- |
| Live child failure/cancellation, restart, replay, and cache-eviction scenarios | **Deferred as acceptance scenarios.** The local two-binary target adds exact cancellation of one outstanding long-running execution, but that six-run assertion is not live-verified; child failure/cancellation and broader recovery cases remain planned. | [Live acceptance coverage](live-acceptance-coverage.md) |
...
| Live activity heartbeat acceptance | **Implemented as a focused native slice, not live-verified.** Context-aware activities can read the previous attempt's details, inspect the server heartbeat timeout, and send typed progress through the serialized supervisor. Strict bilateral JSON validation, lease retention, and post-completion context invalidation are covered by local tests. | A real Temporal Server heartbeat/retry/timeout scenario remains planned. [Native activity execution](native-activity-execution.md), [activity protocol](activity-protocol.md) |
