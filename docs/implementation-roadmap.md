...
| 6 | Continue-as-new, patches, side effects, external workflow operations, memo, search attributes, priority, and fairness | Recorded histories replay and advanced command integration tests pass | In progress: continue-as-new is implemented and live-verified. The first non-deprecated `Temporal.Workflow.patched` primitive, bilateral JSON validation, per-execution replay decisions, Core conversion, closed acceptance schemas, and a Docker-free contract are implemented. The complete [PR #348 CI run](https://github.com/mfow/ocaml-temporal/actions/runs/29411260374) live-verifies both marker-free legacy and marker-bearing new histories through worker replacement. Patch deprecation/removal, side effects, external operations, memo/search attributes, priority, fairness, and broader CI evidence remain pending |
