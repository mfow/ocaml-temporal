...
/// Encodes and reparses a positive termination acknowledgement before it
/// crosses the native boundary.
...
/// Terminates one exact run through Temporal's official workflow service.
/// This is intentionally distinct from cancellation: Temporal records a
/// termination command immediately and does not wait for workflow code to
/// handle cancellation. The one-second bound keeps the owner Domain
/// responsive when the control-plane RPC stalls.
...
mod tests {
    ...
    /// Builds a valid exact-run termination request used by strict protocol
    /// tests.
    fn terminate_json() -> String {
        serde_json::json!({
            "namespace":"default",
            "workflow_id":"workflow-1",
            "run_id":"run-1",
    ...
    #[test]
    /// Termination has an exact-run request shape independent from
    /// cancellation's request ID while retaining strict reason validation.
    fn termination_request_and_acknowledgement_are_closed_and_bounded() {
        let request =
            decode_terminate_request(&terminate_json()).expect("terminate request decodes");
        assert_eq!(request.namespace, "default");
        assert_eq!(request.workflow_id, "workflow-1");
        assert_eq!(request.run_id, "run-1");
        assert_eq!(request.reason, "operator requested termination");
        assert_eq!(
            encode_terminate_response(&TerminateWorkflowResponse { acknowledged: true })
                .expect("positive termination acknowledgement encodes"),
            r#"{"acknowledged":true}"#
        );

        let mut unknown = terminate_json();
        unknown.insert_str(unknown.len() - 1, ",\"unexpected\":true");
        assert!(decode_terminate_request(&unknown).is_err());
        let duplicate = r#"{"namespace":"default","workflow_id":"workflow-1","run_id":"run-1","run_id":"other","reason":""}"#;
        assert!(decode_terminate_request(duplicate).is_err());
        let nul_reason = serde_json::json!({
            "namespace":"default",
            "workflow_id":"workflow-1",
            "run_id":"run-1",
        ...
        .to_string();
        assert!(decode_terminate_request(&nul_reason).is_err());
        let oversized_reason = serde_json::json!({
            "namespace":"default",
            "workflow_id":"workflow-1",
            "run_id":"run-1",
        ...
        .to_string();
        assert!(decode_terminate_request(&oversized_reason).is_err());
        assert!(
