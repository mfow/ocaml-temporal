...
- **Verified live** means a test used the real Temporal Server and PostgreSQL
  Compose stack. The historical [PR #210 CI run](https://github.com/mfow/ocaml-temporal/actions/runs/29221151859)
  verifies the original two-binary path for nine scenarios. The current local
  OCaml 5.5 run also covers the delayed asynchronous activity, timeout retry,
  and continue-as-new successor paths; it passed all twelve top-level
  assertions against the Compose stack. That local result is evidence for this
  checkout only until the corresponding CI run completes.
...
| Public client start, exact-run wait, cancellation, and typed signal | A separate OCaml driver starts eleven top-level workflows before its first wait through `Temporal.Client`, retains all returned handles, then starts the timeout-retry workflow after the heartbeat result. It waits for every exact workflow/run pair, acknowledges cancellation for the long-running handle, and exposes `Temporal.Client.signal` for one exact run with a typed `Temporal.Signal` definition. | **Focused-tested** locally for typed request validation, exact identity, acknowledgement/error decoding, and mock lifecycle behavior. The live acceptance driver currently covers start, wait, and cancellation; live signal acceptance remains pending. |
| Public worker workflow and activity dispatch | A separate OCaml worker registers workflows and activities for ordinary retry, context-aware heartbeat retry, delayed asynchronous completion, timeout retry, child failure/cancellation, typed failure, continue-as-new, and cancellation-marker paths through real Core polling/completion. | **Verified live locally** by the same OCaml 5.5 Compose run. The historical [PR #210 run](https://github.com/mfow/ocaml-temporal/actions/runs/29221151859) remains the linked CI evidence for the earlier slice. |
...
| Private replay worker and history feeder | **Unit-tested native plumbing plus implemented live acceptance controller.** `rust/core-bridge/src/replay_bridge.rs` validates the closed replay-history JSON document, canonical base64 protobuf, and Core history invariants, then feeds histories through a one-slot workflow-only `HistoryFeeder`; focused tests live in [`rust/core-bridge/tests/support/replay_bridge.rs`](../../rust/core-bridge/tests/support/replay_bridge.rs). The OCaml worker activation hook, strict history normalizer, generation-aware diagnostics, and ordered controller contract are implemented and contract-tested. | No successful live restart/replay result is recorded yet. `make test-temporal-worker-restart-live` is the real-server gate; sticky-cache eviction and a public replay API remain separate work. |
...
| Native client start, exact-run wait, cancellation, and signal | The private [client protocol](client-protocol.md), Rust client bridge, supervisor operations, request correlation, bounded waits, positive control acknowledgements, terminal outcome mapping, and typed public handles are implemented. Cancellation and signal use the exact retained run identity and bounded native RPCs; signal payloads are validated in both languages before the official `SignalWorkflowExecution` call. | **Focused-tested** for signal JSON shape, status correlation, ABI state guards, and mock/public request handling. Start, exact-run wait, marker-guarded cancellation, and continue-as-new successor following are verified live locally; live signal acceptance remains pending. |
