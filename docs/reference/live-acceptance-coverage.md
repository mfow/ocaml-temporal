...
Docker-free contract has two layers: `test/smoke/test_temporal_heartbeat_contract.sh`
checks that the independent driver starts and asserts the heartbeat workflow
and that the independent worker registers the context-aware activity, while
`test/integration/temporal/common/marker_test/test_smoke_definitions.ml` invokes
that exact activity callback twice with in-memory contexts. The latter checks
the encoded progress payload, the retryable first failure, the 500 ms timeout,
the detail delivered to the second context, and rejection after the first
context is invalidated. These tests prove the fixture's local contract without
claiming that a fake context observed a Temporal Server retry. The
...
child failure and the client driver checks the resulting public `Workflow`
terminal category and retryability flag without depending on Core's diagnostic
wording. The native worker path separately checks the `Child_workflow` category
when the parent observes the child future.
