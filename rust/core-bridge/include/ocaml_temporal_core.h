...
/* Request immediate termination of one exact workflow run. The successful
 * value is a strict {"acknowledged":true} document. */
ocaml_temporal_core_status ocaml_temporal_core_v1_client_terminate_workflow_json(
    ocaml_temporal_core_runtime *runtime, const uint8_t *input,
    size_t input_len, ocaml_temporal_core_result *output);
