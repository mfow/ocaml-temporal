...
outside the current thirteen-result fixture. The restart/replay controller is
...
the local thirteen-result run is live evidence for the successful exact-run
...
| A workflow starts, runs, and returns a terminal result | Synthetic activation and native adapter tests cover command construction and terminal handling. | **Verified (live success path locally).** The OCaml 5.5 Compose run starts twelve workflows before the first wait, waits for the signal workflow's worker-visible readiness marker before signaling it, starts the timeout-retry workflow after heartbeat completion, and asserts all thirteen exact results or typed terminal outcomes, including continue-as-new. | Terminated outcomes remain pending. |
...
| Multiple operations scheduled before awaiting | **Verified (synthetic only).** Scheduler and activation tests cover completion ordering, first-error behavior, and cancellation semantics. | **Verified locally (live success path).** `smoke.fan_out` schedules two activities before its first wait; the cancellation workflow schedules its timer and marker activity before waiting; and the driver starts twelve top-level workflows before awaiting any result. It waits for the signal workflow's readiness marker before signaling, then starts the timeout workflow after heartbeat completion and asserts all thirteen results. | `race` and explicit server-history ordering assertions are **Planned — later expansion**. |
...
start a real Temporal Server. The former owns the thirteen-result baseline; the
...
cache-eviction, or recovery scenarios. The [PR #253 run](https://github.com/mfow/ocaml-temporal/actions/runs/29286560471)
