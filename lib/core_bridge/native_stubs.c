...
/* Request immediate termination of one exact workflow run. The JSON input is
 * copied before the OCaml runtime lock is released; Rust owns no OCaml memory
 * after this call returns. */
CAMLprim value ocaml_temporal_client_terminate_workflow_json(value runtime,
                                                             value input) {
  return invoke_runtime_json(
      runtime, input, ocaml_temporal_core_v1_client_terminate_workflow_json);
}
