...
fan-out and timer/activity cases. The current driver starts six workflows
...
activity task is delivered by the live server/Core path. The fifth workflow
...
for merge commit `a4eaccc8` verifies those five baseline executions. The
current acceptance change adds the sixth cancellation scenario; its driver
waits for a test-only marker activity after the timer and marker commands are
issued together, then cancels the exact run before waiting for typed
`Cancelled` metadata. Retry-policy construction, JSON representation, and
Core conversion remain separately covered by synthetic tests. Retry timeouts,
activity-level non-retryable error-type classification, replay, and recovery
remain outside the live gate.
...
Rust protocol tests. The live driver starts a long-running durable-timer
workflow, waits for its test-only marker activity to publish the current run
token after the timer and marker commands are issued together, then sends
`Temporal.Client.cancel` using the returned exact handle. It waits on that same
handle and checks the `Cancelled` category, retryability, and stable message.
The driver and worker also report successful shutdown phases; the Makefile
checks both markers before removing the Compose project.
...
| A workflow starts, runs, and returns a terminal result | Synthetic activation and native adapter tests cover command construction and terminal handling. | **Five baseline executions verified** in CI run [`29191260073`](https://github.com/mfow/ocaml-temporal/actions/runs/29191260073). The driver retains exact workflow/run handles and starts six top-level workflows before the first wait; this change adds the marker-guarded cancellation scenario. | Timed-out, terminated, and continued-as-new outcomes remain untested live. |
